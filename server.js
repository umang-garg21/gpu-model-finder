...
function renderModelPage(entry) {
  ...
  const canonicalUrl = `https://byog.up.railway.app/models/${entry.slug}`;
  const seoTitle = `${entry.name} GPU Requirements — VRAM Calculator | BYOG: Bring your own GPU`;
  ...
  const jsonLdBreadcrumb = JSON.stringify({
    ...
    '@type': 'BreadcrumbList',
    'itemListElement': [
      { '@type': 'ListItem', 'position': 1, 'name': 'Home', 'item': 'https://byog.up.railway.app/' },
      { '@type': 'ListItem', 'position': 2, 'name': 'Models', 'item': 'https://byog.up.railway.app/models' },
...
app.get('/models', (req, res) => {
  ...
  res.type('text/html').send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Model GPU Requirements — BYOG: Bring your own GPU</title><meta name="description" content="GPU VRAM requirements for 50+ popular AI models including Llama, Mistral, Qwen, DeepSeek, Gemma and more."><link rel="canonical" href="https://byog.up.railway.app/models"><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap" rel="stylesheet"></head><body style="background:#0a0a0a;color:#e0e0e0;font-family:Inter,sans-serif;margin:0"><nav style="position:sticky;top:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:0 2rem;height:60px;background:rgba(10,10,10,0.85);backdrop-filter:blur(12px);border-bottom:1px solid #1e1e1e"><a href="/" style="font-weight:700;color:#e0e0e0;text-decoration:none"><span style="color:#76b900">◈</span> BYOG: Bring your own GPU</a><a href="/tool" style="background:#76b900;color:#0a0a0a;font-weight:700;font-size:0.85rem;padding:0.45rem 1.1rem;border-radius:6px;text-decoration:none">Launch Tool →</a></nav><div style="max-width:960px;margin:0 auto;padding:2rem 1.5rem"><div style="margin-bottom:0.5rem;font-size:0.82rem;color:#606060"><a href="/" style="color:#606060">Home</a> › Models</div><h1 style="font-size:2rem;font-weight:800;letter-spacing:-0.025em;margin-bottom:0.5rem">AI Model GPU Requirements</h1><p style="color:#606060;margin-bottom:2rem">VRAM requirements, benchmark scores, and GPU compatibility for 50+ popular open-source models.</p>${sections}</div><footer style="padding:2rem;border-top:1px solid #1e1e1e;text-align:center;color:#606060;font-size:0.82rem">Model data from <a href="https://huggingface.co" target="_blank" style="color:#606060">HuggingFace</a> &amp; <a href="https://artificialanalysis.ai" target="_blank" style="color:#606060">ArtificialAnalysis</a>. BYOG: Bring your own GPU is free and open-source.</footer></body></html>`);
});
...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"BYOG: Bring your own GPU Contact" <${smtpUser}>`,
...
app.post('/api/subscribe', async (req, res) => {
  ...
  if (smtpUser && smtpPass) {
    try {
      ...
      await transporter.sendMail({
        from: `"BYOG: Bring your own GPU" <${smtpUser}>`,
        to: 'umanggarg78@gmail.com',
